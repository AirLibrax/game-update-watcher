...
- **6 周版本节奏**：按版本更新日推算当前处于哪个阶段，展示上下半池/前瞻/下版本时间（栏位数因游戏而异）
...
| **指令 `/游戏速报`** | 全部游戏汇总图 | 无需配置 |
| **指令 `/游戏速报 游戏名`** | 单游戏信息图（支持别名，如 `终末地`、`明日方舟终末地`） | 无需配置 |
...
每个游戏按自己的节奏展示栏位，随版本进度推进：

**版本制游戏（鸣潮/崩铁/绝区零/终末地）两个栏位：**
...
**推算核心公式（timeline.py 的 build_timeline）**：

```
下版本更新时间 = 版本更新日 + cycle_days        （有官方 end_time 时用官方值）
前瞻时间       = 下版本更新时间 - preview_ahead_days
下半池开启     = 版本更新日 + half_days          （有官方卡池公告时用官方值）
阶段           = (今天 - 版本更新日) / 7 周
```

**每款游戏节奏不同，参数由 `games/*.json` 自定义**：

| 游戏 | cycle_days（大版本天数） | half_days（下半池切换） | preview_ahead_days（前瞻提前） | 模式 |
|---|---|---|---|---|
| 鸣潮 | 35 | 18 | 7 | 版本制 |
| 崩铁 | 42 | 21 | 7 | 版本制 |
| 绝区零 | 42 | 21 | 7 | 版本制 |
| 终末地 | 42 | 21 | 7 | 版本制 |
| 明日方舟 | 28 | — | — | **活动制**（activity_mode=true，不走周数） |

**优先级：官方数据 > 推算**。每个关键时间点都优先用官方公告值（`end_time`、卡池公告、`known_dates` 配置），只有拿不到官方值时才用上面的公式推算，推算值标「（预估）」。

- **确定 vs 预估**：官方公告时间（含 end_time、卡池公告、known_dates、活动制 activity_end）不标预估；周期推算（版本更新日 + cycle_days）标「（预估）」
- **活动制结束日**：方舟等活动制游戏优先取正文「开放时间」段的官方结束日（如 SideStory 关卡 8/22 结束），无官方时间才用周期推算
- 方舟是活动制（无版本号）：栏位A=本版本活动+新干员，栏位B=限定寻访（官方时间），栏位C=下版本预告（官方公布才显示，否则显示“未知”）

### 5. 渲染层：Pillow 出图（renderer.py）

- `render_summary`：多游戏汇总长图，每款一个区块（游戏名+版本标题+按游戏配置的栏位），一次发送
...
2. 确认依赖：`pip install httpx pillow`（MaiBot 通常已内置）
3. 重启 MaiBot，日志出现“加载完成”即成功

> 插件带默认配置，开箱即用（Tool/指令触发无需任何配置）。如需调整（定时推送、跟踪游戏等），复制 `config.example.toml` 为 `config.toml` 后编辑，或通过 WebUI 插件设置页修改。
...
**验证脚本说明**：
- `verify.py` 是本地开发自检工具，通过 `subprocess` 以固定参数调用 `selftest.py`，**不接收任何用户输入**，仅用于确认插件在部署前能正常采集和出图
- 两者都不参与插件运行时逻辑，部署到 MaiBot 后不会被执行
- 运行需要联网（采集官方公告接口），离线环境会显示采集失败，属预期行为
...
└── core/
    ...
    ├── timeline.py       # 版本节奏判定 + 栏位生成（版本制/活动制）
